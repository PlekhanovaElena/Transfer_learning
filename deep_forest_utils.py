...
#         self.base_estimator = prob_class()
    ...
    def fit(self,X_train,y_train, sample_weight=None):
        ...
        for estimator in estimators:
            estimator.fit(X_train, y_train, sample_weight = sample_weight) #### weighted
            if 5 > 1:# use cv ### ALWAYS
                predict_ = cvp(estimator, X_train, y_train, cv=self.k_fold, n_jobs = -1)
                #predict_ = cvp(estimator, X_train, y_train, cv=5, n_jobs = -1)
            else:#use oob
        ...
        self.score = self.max_accuracy

        #cascade step
        while True:
            print('level %i, CV accuracy: %f' % (len(self.estimators_levels),self.max_accuracy))
            estimators = [klass(**params) for params in self.params_list]
            self.estimators_levels.append(estimators)
            predictions = []
            X_train_step = np.hstack((attr_to_next_level,X_train))
            for estimator in estimators:
                estimator.fit(X_train_step, y_train, sample_weight = sample_weight)  #### weighted
                if 5 > 1:# use cv ### ALWAYS
    ...
    def predict_proba_staged_cv(self,X, y):
        ...
        for estimator in estimators:
            ...
            predict_ = cvp(estimator, X, y, cv = self.k_fold)
            predictions.append(predict_)
        attr_to_next_level = np.hstack(predictions)
        ...
        for i in range(1,len(self.estimators_levels)):
            ...
            for estimator in estimators:
                ...
                predict_ = cvp(estimator, X_step, y, cv = self.k_fold)
                predictions.append(predict_)
            attr_to_next_level = np.hstack(predictions)
    ...
    def predict_proba_cv(self,X,y):
        return self.predict_proba_staged_cv(X,y)[-1]
